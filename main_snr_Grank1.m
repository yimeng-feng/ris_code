...
for mm = 1:numMC
    ...
    for indxSnrDb = 1:length(snrDbSet)
        ...
        RpsInf(indxSnrDb,mm) = RIS_grank1(G,H,snrLin);
%         RpsInf1(indxSnrDb,mm) = RIS(G,H,snrLin,Ns,Hd,'psInf');
...
% plot(snrDbSet,mean(RpsInf1,2),'k--','LineWidth',width), hold on
plot(snrDbSet,mean(Ropt,2),'k','LineWidth',width), hold on
legend('Our proposed algorithm (b=1)','Our proposed algorithm (b=2)','Our proposed algorithm (b=\infty)','The Upper Bound','Location','NorthWest')%'RIS scheme (b=1)','RIS scheme (b=2)',
grid on
xlabel('SNR (dB)')
